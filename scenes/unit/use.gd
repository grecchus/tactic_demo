extends Action
#Code for "Use" node. This will check for unit's item and perform action it's responsible for,
...
func perform(ap_restraint, target_pos = null) -> void:
	...
	get_owner()._on_action_started(1,false) #changed false to true!!! CRITICAL
